...
// GET /api/employees/attendance/user/:id ?month=YYYY-MM
router.get('/attendance/user/:id', requireRole('admin', 'accountant'), async (req, res) => {
  try {
    const { month } = req.query;
    if (!month) return res.status(400).json({ message: 'month YYYY-MM is required' });
    const [rows] = await pool.query(
...
router.get('/:id/attendance', requireRole('admin', 'accountant'), async (req, res) => {
  try {
    const { month } = req.query;
    if (!month) return res.status(400).json({ message: 'month YYYY-MM is required' });
    const [rows] = await pool.query(
...
// GET /api/employees/attendance/report ?month=YYYY-MM
router.get('/attendance/report', requireRole('admin', 'accountant'), async (req, res) => {
  try {
    const { month } = req.query;
    if (!month) return res.status(400).json({ message: 'month YYYY-MM is required' });
    ...
    const [attendance] = await pool.query(
      `SELECT user_id, DATE_FORMAT(date, "%Y-%m-%d") AS date, status, notes FROM attendance WHERE DATE_FORMAT(date, "%Y-%m") = ?`,
      [month]
    );
